...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    uint8_t out = (rx_data == '\r' || rx_data == '\n') ? rx_data : (uint8_t)ECHO_CHAR;

    HAL_UART_Transmit(&huart2, &out, 1, 10);   // Enviar # o $, o CR/LF si aplica
    HAL_UART_Receive_IT(&huart2, &rx_data, 1); // Re-armar recepción del siguiente byte
  }
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  char *welcome_msg = "El sistema de Control Basico esta listo\r\n";
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 200) // Cada 500 ms
    {
    ...
    if (button_flag == 1)
    {
      ...
      sprintf(tx_buffer, "Boton B1 presionado!. LED externo encendido. \r\n");
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      sprintf(tx_buffer, "LED EXT Apagado (Se acababo el tiempo).\r\n");
      HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
